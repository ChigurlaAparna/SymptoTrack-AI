...
class SymptomExtractor:
    ...
    def __init__(self):
        self.symptom_patterns = {
            "fever": r"(?:have\s+)?(?:a\s+)?fever|high?\s+temperature|febrile|chills?",
            "cough": r"cough(?:ing)?|dry\s+cough|wet\s+cough",
            "headache": r"headache|head\s+pain|head\s+ache|migraine",
            "fatigue": r"fatigue[sd]?|tired(?:ness)?|exhaust(?:ed|ion)|weak(?:ness)?|letharg",
            "nausea": r"nausea|nauseous|feel(?:s)?\s+sick|queasy",
            "vomiting": r"vomit(?:ing)?|throw(?:ing)?\s+up|being\s+sick",
            "diarrhea": r"diarrhea|loose\s+stool|watery\s+stool",
            "sore_throat": r"sore\s+throat|throat\s+pain",
            "runny_nose": r"runny\s+nose|nasal\s+discharge|rhinorrhea",
            "congestion": r"congestion|stuffy\s+nose|blocked\s+nose",
            "body_aches": r"body\s+ache|body\s+pain|muscle\s+(?:pain|ache)|ache[sd]?",
            "breathing": r"(?:shortness|difficulty)\s+of\s+breath|breathless|dyspnea|breathing\s+problem",
            "chest_pain": r"chest\s+(?:pain|tightness)",
            "dizziness": r"dizz(?:y|iness)|lightheaded",
            "rash": r"rash|skin\s+rash|redness",
            "itching": r"itch(?:ing|y)|pruritus",
            "sneezing": r"sneezing|sneeze",
            "wheezing": r"wheezing|wheeze",
            "abdominal_pain": r"(?:abdominal|stomach|belly)\s+pain|belly\s+ache|stomach\s+ache",
            "back_pain": r"back\s+pain|lower\s+back\s+pain",
            "joint_pain": r"joint\s+pain|arthralgia",
            "swelling": r"swelling|swollen|edema",
            "numbness": r"numb(?:ness)?|tingling|pins\s+and\s+needles",
            "blurred_vision": r"blurred?\s+vision|vision\s+problem",
            "loss_of_taste": r"loss\s+of\s+taste|ageusia",
            "loss_of_smell": r"loss\s+of\s+smell|anosmia",
            "frequent_urination": r"frequent\s+urination",
            "burning_urination": r"burning\s+(?:when\s+)?urinating|painful\s+urination",
            "anxiety": r"anxi(?:ety|ous)|worried?|nervous",
            "depression": r"depress(?:ed|ion)|sad|feeling\s+down",
            "insomnia": r"insomnia|can'?t\s+sleep|trouble\s+sleeping",
            "weight_loss": r"weight\s+loss",
            "night_sweats": r"night\s+sweats",
        }
